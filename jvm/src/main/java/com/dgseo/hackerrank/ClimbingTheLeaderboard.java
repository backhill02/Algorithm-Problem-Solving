...
public class ClimbingTheLeaderboard {
    public static void main(String[] args) {
        ...
        Arrays.stream(climbingLeaderboard(scores, alice)).forEach(System.out::println);
    }
    ...
    private static int getRank(List<Integer> ranks, int l, int r, int score) {
        if (r - l <= 1) {
            if (ranks.get(l) == score) return l + 1;
            else return r + 1;
        }
        int mid = (l + r) / 2;
        if (ranks.get(l) >= score && score >= ranks.get(mid)) {
            return getRank(ranks, l, mid, score);
        } else if (ranks.get(mid + 1) >= score && score >= ranks.get(r)) {
            return getRank(ranks, mid + 1, r, score);
        } else {
            return mid + 2;
        }
    }
